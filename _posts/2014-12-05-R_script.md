...
In the previous work, the mutants (single, double and triple) are plotted with three variables: folding free energy at 37C (dGflex_37C), 
temperature senstivity normalized score (TS_delta_norm_socre), and GFP seq activity (delta2_reflow_normalized).  
![scatter plot][plot1]
In this work, the mutants that are considered as outliers are extracted from the original data set. 
The outliers that are located in the upper left Quadran and lower right Quadran, where temperature senstivity is not able to be explained by folding free energy at 37C.  
